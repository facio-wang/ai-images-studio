...
function convertRouteComponent(
  ...
  // 是否为有子菜单的顶级菜单
  const isTopLevelWithChildren = depth === 0 && (route.children?.length ?? 0) > 0

  if (route.meta.isIframe) {
    handleIframeRoute(converted, route, iframeRoutes)
  } else if (isFirstLevel) {
    handleLayoutRoute(converted, route, component as string)
  } else if (isTopLevelWithChildren) {
    // 有子菜单的顶级菜单：父级用 Home(layout)，子菜单挂到 children 下
    // 注意：不要调 handleLayoutRoute，那个函数会改 path 和加 isFirstLevel meta
    // children 的递归处理已经在后面自动挂载
