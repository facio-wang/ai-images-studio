...
import { uploadService } from '@/api/uploadApi' // 修改导入路径
...
const props = withDefaults(defineProps<UploadFileProps>(), {
  ...
  limit: 5,
  fileSize: 5,
  fileType: () => ['image/jpeg', 'image/png', 'image/gif'] as ImageMimeType[],
  height: '150px',
  width: '150px',
  borderRadius: '8px'
})
...
const handleHttpUpload = async (options: UploadRequestOptions) => {
  ...
  try {
    // 使用 uploadService.upload 代替 uploadImg
    const api = props.api ?? uploadService.upload
...
const uploadSuccess = (response: { url: string } | undefined, uploadFile: UploadFile) => {
  if (!response) return
  // 使用 response.url 而不是 response.fileUrl
