...
| # | 模块 | 路由 | 说明 |
|---|------|------|------|
| 1 | 工作台概览 | `#/studio/home` | 今日任务/资产统计、**系统状态面板**（ComfyUI 状态灯 + GPU 显存/内存仪表，30s 自动刷新）、最近作品、快捷入口 |
| 2 | 对话工作台 | `#/studio/chat` | 自然语言驱动生图/抠图/查询，消息内嵌产物图，产物可查看**生成详情**（完整提示词/模型/参数），失败任务气泡内一键重试，服务未启动横幅提示 |
| 3 | AI 生图 | `#/studio/generate` | ComfyUI 文生图：模型选择（单文件底模 / GGUF 量化底模）、尺寸预设、steps/cfg/seed、LoRA 挂载、批量生成、海报文字叠加 |
| 4 | 智能抠图 | `#/studio/matting` | 上传或选资产一键去背景（bria-rmbg / birefnet），前后对比 + 下载 |
| 5 | 模型中心 | `#/studio/models` | 底模/LoRA/翻译/抠图四类模型统一管理：启停、设默认、一键从 ComfyUI 同步模型列表 |
| 6 | 资产库 | `#/studio/assets` | 生图/抠图/上传产物统一浏览、筛选、下载、删除，点开**详情弹窗**查看生成时的提示词与全部参数 |
| 7 | 任务中心 | `#/studio/tasks` | 异步任务队列跟踪（queued/running/done/failed），**详情弹窗**（提示词/模型/采样参数/失败原因），失败自动重试 + 手动重试 |
...
- 指令式提示词（如"生成一张《铠甲勇士》炎龙侠单人展示图"）自动改写为**描述性画面提示词**，专有名词原样保留，显著提升角色命中率
...
### 异步任务队列
- SQLite 持久化任务表 + asyncio worker 轮询执行，进程重启不丢任务
...
- **抠图**：rembg（bria-rmbg / birefnet），CPU 即可运行
